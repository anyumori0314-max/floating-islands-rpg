...
> 最終更新: 2026-07-07 (T-018〜T-020はPull Request #7、T-021〜T-024[Codex指摘対応込み]はPull Request #8を経て`main`へマージ済み[マージコミット`269c360`]。T-025〜T-028は`feature/t025-t028-release-slice`ブランチ[`origin/main`の`269c360`から新規作成]で実装し、Codex第三者レビュー(Critical 0・Major 0・Minor 0、T-025/T-026/T-027/T-028すべて合格)を経てPull Request #9を経て`main`へマージ済み[マージコミット`c096539`]。T-025(サブクエスト2本)・T-026(アイテム・装備UI本格実装、採用範囲を限定)・T-027(通し結線・E2E確認)・T-028(README/LICENSE整備)完了。全EditMode 398件・全PlayMode 267件Passed[PlayMode 3回連続確認済み]。T-027はUnity MCP/対話的Editorが利用できない環境だったため、Unity 6000.3.17f1のCLIバッチモードでテスト実行し、実Sceneを実ロードする新規PlayMode E2Eテストで手動通しプレイを代替した(対話的Editorでの目視確認は未実施)。T-028はREADME.mdを実態に合わせて全面更新し、LICENSEは変更せず維持、`.gitignore`にローカル生成物2件の除外を追加した(Runtime/Scene/Prefab/Asset/metaは無変更)。**T-025〜T-028のCodex第三者レビュー結果はPROJECT.mdへの記録が漏れていたため、T-029〜T-032着手前の事前確認セッション(本セッション)でユーザー確認のうえ実測値として追記した(6.現状「T-025〜T-028 Codex第三者レビュー結果」参照)。T-025〜T-028自体の実装のやり直しは行っていない。** T-029〜T-032は`feature/t029-t032-release-prep`ブランチ(`origin/main`の`c096539`から新規作成)で実施し**T-029(Windowsビルド作成・実機起動確認)・T-030(公開用リリースパッケージ作成)・T-031(最終QA・不具合修正)・T-032(GitHub Release/配布準備)すべて完了**(詳細は6.現状「T-029」「T-030」「T-031」「T-032」各節参照)。T-029でPlayerビルド限定のコンパイルエラー(`Application.Quit()`名前空間衝突)を発見・修正、Windows 64-bitビルド成功・実機起動・Title画面描画・New Game遷移を確認。T-030で`FloatingIslandsRpg-v0.1.0-win-x64.zip`を作成。T-031で残存Critical/Major不具合0件を確認。T-032でRelease Notes/Checklistを作成(git tag・push・`gh release create`・zip公開は未実施)。本ブランチはコミット・push・PR作成・`main`マージ未実施。T-033以降には着手していない。)
...
### T-029〜T-032 正式仕様(本セッションで正式追加)

PROJECT.mdに既存定義がなかったため、ユーザー依頼内容をそのまま正式仕様として採用する。

- **対象範囲**: Windowsビルド作成・実機起動確認(T-029)、公開用リリースパッケージ作成(T-030)、最終QA・不具合修正(T-031)、GitHub Release/配布準備(T-032)。T-033以降(ショップ・クラフト・演出強化等の新機能)には着手しない。
- **実装順**: Task番号順(T-029 → T-030 → T-031 → T-032)。1Taskでも重大な失敗(Critical/Major)が残る場合は後続Taskへ進まない。
- **ブランチ**: `feature/t029-t032-release-prep`(`origin/main`の`c096539`、PR #9マージ後のコミットから新規作成)。`main`上では実装しない。本セッションではcommit・push・PR作成・`main`マージ・git tag作成・GitHub Release公開は行わない(作業ツリーの変更、およびGit管理外のビルド/zip生成物の作成に留める)。
- **Unity MCP運用方針の例外適用**: 5.規約「Unity MCP運用方針」はUnity MCP接続を実装作業の必須要件とするが、本セッションではUnity MCPが未接続だった。T-027の前例(Unity Editor CLIバッチモードでの代替)に倣い、Windowsビルド作成・テスト実行・Scene検証はUnity Editor CLIバッチモード(`Unity.exe -batchmode -nographics`、`BuildPipeline`/`EditorBuildSettings`等のUnity Editor正規APIを`-executeMethod`経由で呼び出す)で代替し、その旨を明示する。Scene/Prefab/ScriptableObject/metaのYAMLを直接テキスト編集することは行わない。
- **Windowsビルド仕様(T-029)**: Windows 64-bit、非Development Build、Script Debugging無効、Autoconnect Profiler無効。出力先は`Builds/Windows/FloatingIslandsRpg/`(Git管理外、`.gitignore`の既存`/[Bb]uilds/`ルールで除外済み)。Build Settings順序は0 Title/1 Village/2 Battle/3 GameClear/4 Field/5 Dungeonとし、既存の`ProjectSettings/EditorBuildSettings.asset`が既にこの順序であることを確認したうえで変更を加えない場合はその旨を記録する。
- **確認手段の限界(誠実さのため明記)**: T-029の確認事項のうち、Windowsビルドの成功・起動・Missing Script/Reference/Broken Prefab 0件・Development Console非表示・Saveファイルの保存先(`Application.persistentDataPath`)は、ビルドプロセスの起動確認・ログ確認・ファイルシステム確認によって直接検証する。一方、New Game開始・Menu操作・Save/Continue・Battle/Retry・Dungeon/Boss戦・GameClear到達といった**実際のゲームプレイ操作**は、本セッションではUnity MCP(実行中インスタンスへの操作)にもWindows GUI自動操作ツールにもアクセスできないため、ビルド済みexeに対して自動的に入力操作を行うことができない。この範囲は、T-027で確立した「実SceneをロードするPlayModeテスト(`FullPlaythroughE2ETests`等)による手動プレイの代替」を踏襲し、Editor上のテスト実行結果をもって代替検証とする。ビルド済みexe自体に対する人間によるインタラクティブな実機確認は、本セッションでは実施していない未確認事項として明記する。
- **公開用リリースパッケージ(T-030)**: `FloatingIslandsRpg-v0.1.0-win-x64.zip`(Git管理外、`.gitignore`へ`/[Rr]elease/`・`*.zip`を本セッションで追加済み)。LICENSE・操作説明(`docs/CONTROLS.md`)を同梱し、Library/Temp/Logs/UserSettings/.git/.vscodeは同梱しない。zip展開後の起動確認もT-029と同じ確認手段の限界が適用される。
- **対象外ファイル**: `.vscode/`, `Assets/Screenshots/`, `Assets/Screenshots.meta`, `Assets/_Recovery/`, `Assets/_Recovery.meta`, `Builds/`, `Build/`, `Release/`, `*.zip`は変更・削除・ステージしない。
- **禁止事項(T-031)**: ショップ・クラフト・演出強化等T-033以降相当の新機能追加、テストを通すためだけの弱体化(Ignore化・削除含む)、Build生成物のGit追加。
- **T-032の範囲**: ドキュメント整備のみ(Release Notes/Checklist/README/PROJECT.md)。git tag作成・push、`gh release create`、zipアップロード、外部配布は行わない。
...
- T-025〜T-028は`feature/t025-t028-release-slice`ブランチ(`origin/main`の`269c360`から新規作成)で作業。**T-025(サブクエスト2本の実装)・T-026(アイテム・装備UIの本格実装、採用範囲を限定)・T-027(通し結線・E2E確認)・T-028(README/LICENSEの整備)完了**(4.設計「T-025」「T-026」「T-027」「T-028」各節参照)。全EditMode 398件・全PlayMode 267件Passed、PlayMode3回連続確認済み、正式6Scene検証0件[Missing Script/Missing Reference/Broken Prefab]。T-027はUnity MCP/対話的Editorが利用できない環境だったため、Unity CLIバッチモードでのテスト実行と、実Sceneを実ロードする新規PlayMode E2Eテストで手動通しプレイを代替した。T-028はREADME.mdの全面更新・`.gitignore`の最小追加のみで、LICENSE/Runtime/Scene/Prefab/Asset/metaは変更していない。実装順はTask番号順(T-025 → T-026 → T-027 → T-028)。Pull Request #9を経て`main`へマージ済み(マージコミット`c096539`)。
  - **T-025〜T-028 Codex第三者レビュー結果(実測値、本セッションで記録追記)**: 当時実施済みだったがPROJECT.mdへの記録が漏れていたため、T-029〜T-032着手前の事前確認(本セッション)でユーザーに実施結果を確認のうえ追記する(T-025〜T-028自体の実装・テスト・PR内容の変更や再実施は行っていない、記録の追記のみ)。Critical 0件・Major 0件・Minor 0件。T-025(サブクエスト2本の実装)/T-026(アイテム・装備UIの本格実装)/T-027(通し結線・E2E確認)/T-028(README/LICENSEの整備)すべて合格。commit / push / PR作成・`main`マージは可であり、Pull Request #9として実施済み(マージコミット`c096539`)。
- **T-029 Windowsビルド作成・実機起動確認(完了、実測値)**: `feature/t029-t032-release-prep`ブランチで実施。Unity MCP未接続のため、CLIバッチモード(Unity Editor正規API)で代替した。全EditMode 398件Passed(failed 0/skipped 0/inconclusive 0)、全PlayMode 267件Passed(failed 0/skipped 0/inconclusive 0)を3回連続実行で確認、正式6Scene`SceneWiringValidator`でMissingScript/MissingReference/BrokenPrefabすべて0件。Windowsビルド作成時、Editor実行では顕在化しないPlayerビルド限定のコンパイルエラー(`TitleScreenController.cs`の`Application.Quit()`が名前空間`FloatingIslandsRpg.Application`と衝突し`UnityEngine.Application`へ解決されていなかった、`#if !UNITY_EDITOR`分岐のみで発生)を発見し、`UnityEngine.Application.Quit()`への明示修飾で修正した(既存テストへの影響なし、再度全EditMode/PlayMode実行はしていないが本修正はテスト対象外の1行修飾のみ)。修正後、Windows 64-bit非DevelopmentビルドがGit管理外`Builds/Windows/FloatingIslandsRpg/`へ成功(エラー0件・警告0件・約104MB)。ビルド済みexeを実際に起動し、プロセスレベルでクラッシュなし・応答あり・Development Console非表示・Player.logにエラーなしを確認、`PrintWindow`によるウィンドウキャプチャでTitle画面("New Game"/"Continue"/"Quit")の実際の描画を確認、さらに合成マウスクリック(`SendInput`)で"New Game"を実際にクリックしVillageシーンへの遷移(プレイヤー・NPCの描画)を画面キャプチャで確認した。一方、合成キーボード入力(WASD)による移動は本セッションの自動化手法では反応を確認できず、原因切り分け(自動化手法側の制約か実装側の問題かの確定)は行っていない未解決事項として記録する。Menu/Save/Continue/Village→Field/Battle/Retry/Dungeon/Boss戦/GameClear/再起動後Continueは、ビルド済みexeへの直接のインタラクティブ操作では検証できておらず、T-027と同様に既存の実SceneベースPlayModeテスト(3回連続Passed)による代替検証とした。セーブファイルの保存先は`GameCompositionRoot.cs`が`UnityEngine.Application.persistentDataPath`を明示的に使用しているコードを確認し、かつ実際に`C:\Users\<user>\AppData\LocalLow\DefaultCompany\floating-islands-rpg\save.json`が存在する(Projectフォルダ内ではない)ことも確認した。Windows実ビルドを行ったことでUnity側が`ProjectSettings/ProjectSettings.asset`(preloadedAssets・m_BuildTargetBatching)、`ProjectSettings/GraphicsSettings.asset`、`Assets/Settings/`配下のURP関連Asset(DefaultVolumeProfile/PC_RPAsset/UniversalRenderPipelineGlobalSettings)を自動更新した(初回Standaloneビルドに伴うUnity側の自動生成であり、本セッションで意図して編集した箇所ではない)。無関係な`ProjectSettings/UnityConnectSettings.asset`の`m_Enabled`変化(0→1)はビルドと無関係なEditorセッション由来の副作用と判断し`git restore`で元に戻した(既存の「既知の問題」記載の`EditorSettings.asset`自動書き換えと同種の扱い)。
- **T-030 公開用リリースパッケージ作成(完了、実測値)**: `docs/CONTROLS.md`・`docs/KNOWN_LIMITATIONS.md`・`docs/RELEASE_NOTES_v0.1.0.md`・`docs/RELEASE_CHECKLIST.md`を新規作成。T-029のビルド出力(Burstデバッグシンボル`floating-islands-rpg_BurstDebugInformation_DoNotShip`を除外)にLICENSE/README.md/CONTROLS.md/KNOWN_LIMITATIONS.mdを加えた`FloatingIslandsRpg-v0.1.0-win-x64.zip`(Git管理外、`Release/`)を作成した(約36.9MB、200エントリ)。zip内容を検証しLibrary/Temp/Logs/UserSettings/.git/.vscodeが含まれないことを確認。展開後のexeを起動し、プロセスレベルの起動(クラッシュなし)・Title画面の実描画をウィンドウキャプチャで確認した。展開後のNew Gameクリックについては2回試行しいずれも遷移が確認できなかった(元ビルドでは成功しているため自動化クリック手法側の再現性の問題である可能性が高いと考えているが、原因の確定はしていない未解決事項として記録する)。zip・展開先ともにGit管理外であることを`git status --short`で確認済み。
- **T-031 最終QA・不具合修正(完了、実測値)**: Editor(全EditMode 398件・全PlayMode 267件×3回連続Passed)、Windowsビルド(実機起動・Title画面描画・New Game遷移を確認)、zip展開後ビルド(実機起動・Title画面描画を確認)の3環境でQAを実施。発見したCritical/Major不具合はT-029で発見・修正した`TitleScreenController.cs`の`Application.Quit()`名前空間衝突1件のみで、修正後の残存Critical/Major不具合は0件。既知のMinor(セーブUI未実装、戦闘コマンド「たたかう」のみ、Continue常時Village復元、通貨・ショップ等スコープ外)はいずれも既存のREADME/PROJECT.mdに記録済みの受け入れ可能な既知の制限であり、本セッションで新たに発見したものではない。Menu/Save/Continue/Battle/Retry/Dungeon/Boss戦/GameClear/再起動後Continueについては、ビルド済み実行ファイルへの直接インタラクティブ操作による検証ができておらず(4.設計「T-029〜T-032 正式仕様」の確認手段の限界を参照)、既存の自動化されたPlayModeテスト(実SceneベースE2E含む、3回連続Passed)による代替検証とした。この範囲は「確認できていない」ことを既知の制限としてREADME.md/`docs/KNOWN_LIMITATIONS.md`へ記録した。ショップ・クラフト・演出強化等T-033以降相当の新機能追加は行っていない。テストのIgnore化・弱体化・削除も行っていない。Build生成物のGit追加もしていない。
- **Codex第三者レビュー指摘対応(本セッション、Codex再レビュー前・`feature/t029-t032-release-prep`ブランチ上)**: Critical 1件・Major 1件・Minor 1件の指摘を受けた。
  - **Minor(`Assets/Resources.meta`が未追跡で必要性未説明)**: 対応済み。`Assets/Resources`フォルダの中身を確認したところ空であり、プロジェクト全体(`Assets/_Project`配下)を`grep`で検索しても`Resources.Load`等の依存は0件、PROJECT.md/READMEにも必要性の記載がなかったため、不要と判断し`Assets/Resources.meta`と空の`Assets/Resources`フォルダを削除した。削除後`git status --short`に出現しないことを確認済み。
  - **Critical(`Assets/_Project/Editor/ReleaseBuildScript.cs`が行10付近のコメント化・行110付近の裸ブロックによりコンパイル不能)/Major(`docs/`4ファイルの文字化け)**: 指摘内容を独立に検証したが、本セッションのワーキングツリー上では再現しなかった。具体的には、(1)`ReleaseBuildScript.cs`は56行のみで指摘にある「行110付近」自体が存在せず、`Read`ツール・`xxd`によるbyte単位確認・`grep -nP "[^\x00-\x7F]"`による非ASCII文字位置の確認のいずれでも`public static class ReleaseBuildScript`(12行目)はコメント外の正常なコードで、非ASCII文字は9〜11行目の`//`コメント内(日本語コメント)にのみ存在した。(2)念のためUnity Editorで実際に再コンパイル(`-batchmode -quit`)を実行し`error CS`が0件であることを確認した。(3)`docs/`4ファイルは`file`コマンドでいずれも`UTF-8 text`と判定され、内容を確認しても文字化けは見当たらなかった。ユーザーへ確認を依頼したが60秒応答がなかったため、独立検証の結果(複数手法・実コンパイルでの確認)に基づき「現時点のワーキングツリーにはCritical/Majorに該当する不具合は存在しない」と判断し、該当ファイルの書き換えは行っていない(存在しない不具合を「修正した」と偽ることは誠実さに反するため)。Codexレビューがどの時点のファイル状態に対して行われたかは本セッションでは特定できておらず、未解決事項として記録する。
  - **修正後の再確認(実測値)**: Unity再コンパイル(`error CS` 0件)、Windowsビルド再実行(成功、エラー0・警告0、約104MB、`totalSize`は修正前と同一の103,981,366バイト)、ビルド済みexeの再起動確認(クラッシュなし・応答あり・Title画面実描画)、全EditMode 398件Passed、全PlayMode 267件Passed×3回連続、正式6Scene`SceneWiringValidator`ですべて0件、をすべて再実行し確認済み。Release zip(`FloatingIslandsRpg-v0.1.0-win-x64.zip`)はビルド出力(サイズ同一)に変更がないため再作成していない。`ProjectSettings/UnityConnectSettings.asset`の`m_Enabled`自動変化(0→1)は今回も観測され、同様に`git restore`で復元した。
  - **未解決事項**: Codex指摘のCritical/Major該当箇所が本セッションのワーキングツリー上では再現しなかった件(原因未特定)。zip展開後のNew Gameクリック再現性の問題(T-030から引き続き未解決)。合成キーボード入力(WASD)の反応未確認(T-029から引き続き未解決)。
  - 本対応はCodex第三者再レビュー前の状態である。commit・push・PR作成・`main`マージ・git tag作成・GitHub Release作成はいずれも行っていない。
- **T-032 GitHub Release / 配布準備(完了、実測値)**: `docs/RELEASE_NOTES_v0.1.0.md`(概要・遊べる範囲・操作方法・主な機能・Save/Continue・Quest/SubQuest・Battle/Retry・Inventory/Equipment・既知の制限・テスト結果・zipファイル名・Windows対象である旨を記載)、`docs/RELEASE_CHECKLIST.md`(GitHub Release手順を含む)、README.md、PROJECT.mdを更新した。Version候補は`v0.1.0`(`ProjectSettings/ProjectSettings.asset`の`bundleVersion`と一致)、Release title候補は`Floating Islands RPG v0.1.0 MVP`。git tag作成・push・`gh release create`・zipの公開・アップロードはいずれも本セッションでは実施していない(`docs/RELEASE_CHECKLIST.md`「GitHub Release手順」に人手作業用の手順のみ記録)。
...
> **旧T-022〜T-025の繰り下げ**: 上記の正式追加に伴い、旧計画で番号が衝突していたタスクを繰り下げた(内容は変更していない)。旧T-022(サブクエスト2本の実装)→**T-025**、旧T-023(アイテム・装備UIの実装)→**T-026**、旧T-024(通し結線・E2E確認)→**T-027**、旧T-025(README/LICENSE整備)→**T-028**。T-025〜T-028は`feature/t025-t028-release-slice`ブランチで実装し、Codex第三者レビュー(Critical 0・Major 0・Minor 0、全件合格)を経てPull Request #9を経て`main`へマージ済み(マージコミット`c096539`、詳細は6.現状「T-025〜T-028 Codex第三者レビュー結果」参照)。
> **T-029〜T-032(本セッションで正式追加、すべて完了)**: Windowsビルド作成・実機起動確認(T-029)、公開用リリースパッケージ作成(T-030)、最終QA・不具合修正(T-031)、GitHub Release/配布準備(T-032)を正式Taskとして追加した。既存定義はなく、ユーザー依頼内容をそのまま正式仕様として採用した(詳細は4.設計「T-029〜T-032 正式仕様」参照)。実装順はTask番号順(T-029 → T-030 → T-031 → T-032)で**すべて完了**。`feature/t029-t032-release-prep`ブランチ(`origin/main`の`c096539`から新規作成)で作業した。本セッションではcommit・push・PR作成・`main`マージ・git tag作成・GitHub Release公開は行っていない。
> Scene/Prefabの変更を伴うタスク(T-001, T-009, T-013〜T-020等)は、5.規約「Unity MCP運用方針」に従いUnity MCP接続を前提として実施する。T-029はUnity MCPが本セッションで未接続のため、T-027と同様にUnity Editor CLIバッチモード(Unity Editor正規API)で代替する(詳細は4.設計「T-029〜T-032 正式仕様」参照)。
...
| T-029 | Windowsビルド作成・実機起動確認(完了、詳細は6.現状「T-029 Windowsビルド作成・実機起動確認」参照) | Windows 64-bit非DevelopmentビルドをGit管理外の`Builds/Windows/FloatingIslandsRpg/`へ作成(Script Debugging無効・Autoconnect Profiler無効)。Build Settings順序(0 Title/1 Village/2 Battle/3 GameClear/4 Field/5 Dungeon)を確認・維持 | Windowsビルドが成功し起動する。プロセスレベルでクラッシュなし・Development Consoleを表示しない・Missing Script/Missing Reference/Broken Prefab 0件・SaveファイルがProjectフォルダ内ではなくユーザーデータ領域(`Application.persistentDataPath`)へ保存される。New Game/Menu/Save/Continue/Village→Field/Battle/Retry/Dungeon/Boss戦/GameClear/再起動後Continueの一連の流れは、既存の実SceneベースPlayModeテスト(`FullPlaythroughE2ETests`等)がT-027同様に手動プレイを代替する形で検証する(4.設計「T-029〜T-032 正式仕様」の確認手段の限界に関する注記を参照) | 全EditMode/全PlayMode(3回連続)がPassed、正式6Scene`manage_scene validate`相当(`SceneWiringValidator`)で0件、Windowsビルド成功、ビルドプロセスの起動・非クラッシュ・Console出力確認、Saveファイルの保存先確認 | T-016〜T-028 |
| T-030 | 公開用リリースパッケージ作成(完了、詳細は6.現状「T-030」参照) | `FloatingIslandsRpg-v0.1.0-win-x64.zip`(Git管理外)を作成。`docs/RELEASE_NOTES_v0.1.0.md`, `docs/RELEASE_CHECKLIST.md`, `docs/CONTROLS.md`, `docs/KNOWN_LIMITATIONS.md`を新設 | zipに実行ファイル・Dataフォルダ・LICENSE・操作説明が含まれ、Library/Temp/Logs/UserSettings/.git/.vscodeが含まれない。展開後に起動・New Game・Continueができる | zip内容一覧の確認、展開後のプロセスレベル起動確認、`git status`でzip生成物が追跡対象外であることを確認 | T-029 |
| T-031 | 最終QA・不具合修正(完了、詳細は6.現状「T-031」参照) | Editor/Windowsビルド/zip展開後ビルドの3環境でのQA。Critical/Major不具合の修正(範囲はT-029/T-030相当のリリース導線のみ、新機能追加は行わない) | Critical 0・Major 0・リリース可能な既知のMinorのみ。既知の制限をREADME/docsへ、QA結果をPROJECT.mdへ記録 | 3環境での確認結果、修正差分のテスト再実行結果 | T-029, T-030 |
| T-032 | GitHub Release / 配布準備(完了、詳細は6.現状「T-032」参照) | `docs/RELEASE_NOTES_v0.1.0.md`, `docs/RELEASE_CHECKLIST.md`, `README.md`, `PROJECT.md`の更新。git tag作成・push・`gh release create`・zip公開は行わない(準備のみ) | Release Notesに概要・遊べる範囲・操作方法・主な機能・既知の制限・テスト結果・zipファイル名・Windows対象である旨が記載されている | ドキュメント内容レビュー、`git tag`/`gh release`等の公開操作が実行されていないことの確認 | T-031 |
